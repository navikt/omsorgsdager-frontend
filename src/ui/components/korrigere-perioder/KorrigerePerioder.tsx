...
const hentOmLostAksjonspunktHarBlivitLostHeltEllerDelvis = (
  vilkarOppfylt: boolean,
  dagerInnvilget: number
) => {
...
const KorrigerePerioder: React.FunctionComponent<KorrigerePerioderProps> = ({
  ...
  const methods = useForm<FormData>({
    defaultValues: {
      begrunnelse: harAksjonspunktOgVilkarLostTidligere
      ...
      antallDagerDelvisInnvilget:
        harAksjonspunktOgVilkarLostTidligere &&
        typeof informasjonTilLesemodus.antallDagerDelvisInnvilget !==
          "undefined"
          ? informasjonTilLesemodus.antallDagerDelvisInnvilget
          : null,
    },
    ...
    if (
      ...
      if (data.fravaerGrunnetSmittevernhensynEllerStengt === "delvis") {
        losAksjonspunkt(
          true,
          data.begrunnelse,
          Number(data.antallDagerDelvisInnvilget)
        );
      } else {
        losAksjonspunkt(
          data.fravaerGrunnetSmittevernhensynEllerStengt,
          data.begrunnelse,
          null
        );
      }
  ...
  const erAntallDagerDelvisInnvilgetFyltUt = () => {
    if (getValues().fravaerGrunnetSmittevernhensynEllerStengt !== "delvis")
      return true;
    ...
    return (
      <div className={styleLesemodus.lesemodusboks}>
        <AksjonspunktLesemodus
          aksjonspunktTekst={
            konfliktMedArbeidsgiver
              ? tekstKonfliktMedArbeidsgiver.instruksjon
              : tekst.instruksjon
          }
        ...
        <p className={styleLesemodus.label}>
          {konfliktMedArbeidsgiver
            ? tekstKonfliktMedArbeidsgiver.begrunnelse
            : tekst.begrunnelse}
        </p>
        <p className={styleLesemodus.fritekst}>
          {informasjonTilLesemodus.begrunnelse}
        </p>
      </div>
    );
    ...
    <div className={styles.korrigerePerioder}>
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(bekreftAksjonspunkt)}>
          ...
          {getValues().fravaerGrunnetSmittevernhensynEllerStengt ===
            "delvis" && (
            <div className={styles.antallDagerDelvisInnvilgetContainer}>
              <InputField
                ...
                feil={
                  errors.antallDagerDelvisInnvilget !== undefined
                    ? tekst.feilManglerDager
                    : ""
                }
              />
            </div>
